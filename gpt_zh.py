...
def train_loop():
    ...
    torch.save(model.state_dict(), 'model_final_zh_v2.pt')
...
def load_mode_generate_txt_streaming():
    model = BigramLanguageModel()
    model.load_state_dict(torch.load('model_final_zh_v2.pt', map_location='cpu'))
    model.to(device)
    model.eval()
...
def analy_model():
    ckpt = torch.load('model_final_zh_v2.pt', map_location='cpu', weights_only=True)

    for name, tensor in ckpt.items():
        print(f"{name:45s} {tuple(tensor.shape)}")

def plot_losses(losses_record, save_path='loss_curve_v2.png'):
...
def save_losses_json(losses_record):
    with open('losses_record_v2.json', 'w') as f:
