...
def random_split(data, val_ratio=0.1, chunk_size=1024, seed=1337):
    """按 chunk 随机切分训练/验证集，保持块内序列连续"""
    chunks = [data[i:i+chunk_size] for i in range(0, len(data) - chunk_size + 1, chunk_size)]
    random.seed(seed)
    random.shuffle(chunks)
    n_val = int(len(chunks) * val_ratio)
    val_data = torch.cat(chunks[:n_val])
    train_data = torch.cat(chunks[n_val:])
...
def train_loop():
    ...
    t_train_start = 0.0
    losses_record = []
    best_val = float('inf')   # 历史最优 val loss
    best_iter = -1            # 最优出现在第几步   
    ...
    for iter in range(max_iters):
        if iter % eval_interval == 0 or iter == max_iters - 1:
            ...
            if save_best_model_flag and cur_val < best_val and iter > 24000:
                best_val = cur_val
                best_iter = iter
                torch.save(model.state_dict(), 'model_best_zh_v9.pt')
        ...
        if (iter+1) % save_model_interval == 0:    
            torch.save(model.state_dict(), f'model_ckpt_zh_v9_{iter+1}.pt')    

    t_total = time.time() - t_start
    logging.info(f"全部训练完成，总耗时 {t_total/60:.1f} 分钟，" f"平均每步 {t_total/max_iters*1000:.0f} 毫秒")

    torch.save(model.state_dict(), f'model_final_zh_v9.pt')
...
def load_mode_generate_txt_streaming():
    model = BigramLanguageModel()
    model.load_state_dict(torch.load('model_best_zh_v9.pt', map_location='cpu'))
    model.to(device)
    model.eval()
...
def load_model_for_inference():
    model = BigramLanguageModel()
    model.load_state_dict(torch.load('model_final_zh_v9.pt', map_location='cpu'))
    model.to(device)
    model.eval()
...
def analy_model():
    ckpt = torch.load('model_final_zh_v9.pt', map_location='cpu', weights_only=True)

    for name, tensor in ckpt.items():
        print(f"{name:45s} {tuple(tensor.shape)}")

def plot_losses(losses_record, save_path='loss_curve_v9.png'):
