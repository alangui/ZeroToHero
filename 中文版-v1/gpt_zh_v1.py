...
# 31513480 个参数
# 总样本数据 4076755, 训练集 3669079 验证集 407676
...
with open('AMC_mini/AMC_fictions_100K.txt', 'r', encoding='utf-8') as f:
    text = f.read()
...
def train_loop():
    ...
    torch.save(model.state_dict(), 'model_final_zh_v1.pt')
...
def load_mode_generate_txt_streaming():
    model = BigramLanguageModel()
    model.load_state_dict(torch.load('model_final_zh_v1.pt', map_location='cpu'))
    model.to(device)
    model.eval()
...
def analy_model():
    ckpt = torch.load('model_final_zh_v1.pt', map_location='cpu', weights_only=True)

    for name, tensor in ckpt.items():
        print(f"{name:45s} {tuple(tensor.shape)}")

def plot_losses(losses_record, save_path='loss_curve_v1.png'):
...
def save_losses_json(losses_record):
    with open('losses_record_v1.json', 'w') as f:
