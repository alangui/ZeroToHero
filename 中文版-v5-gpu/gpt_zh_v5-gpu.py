...
def train_loop():
    ...
    for iter in range(max_iters):
        ...
        if (iter+1) % save_model_interval == 0:    
            torch.save(model.state_dict(), f'model_final_zh_v5_{iter}.pt')    

    t_total = time.time() - t_start
    logging.info(f"全部训练完成，总耗时 {t_total/60:.1f} 分钟，" f"平均每步 {t_total/max_iters*1000:.0f} 毫秒")

    torch.save(model.state_dict(), f'model_final_zh_v5.pt')
...
def load_mode_generate_txt_streaming():
    model = BigramLanguageModel()
    model.load_state_dict(torch.load('model_final_zh_v5.pt', map_location='cpu'))
    model.to(device)
    model.eval()
...
def plot_losses(losses_record, save_path='loss_curve_v5.png'):
