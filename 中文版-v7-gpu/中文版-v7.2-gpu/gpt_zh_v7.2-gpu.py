    ...
    for iter in range(max_iters):
        if iter % eval_interval == 0 or iter == max_iters - 1:
            ...
            if save_best_model_flag and cur_val < best_val:
                best_val = cur_val
                best_iter = iter
                torch.save(model.state_dict(), 'model_best_zh_v7.2.pt')
        ...
        if (iter+1) % save_model_interval == 0:    
            torch.save(model.state_dict(), f'model_ckpt_zh_v7.2_{iter+1}.pt')    

    t_total = time.time() - t_start
    logging.info(f"全部训练完成，总耗时 {t_total/60:.1f} 分钟，" f"平均每步 {t_total/max_iters*1000:.0f} 毫秒")

    torch.save(model.state_dict(), f'model_final_zh_v7.2.pt')
...
def load_mode_generate_txt_streaming():
    model = BigramLanguageModel()
    model.load_state_dict(torch.load('model_final_zh_v7.2.pt', map_location='cpu'))
    model.to(device)
    model.eval()
...
def analy_model():
    ckpt = torch.load('model_final_zh_v7.2.pt', map_location='cpu', weights_only=True)

    for name, tensor in ckpt.items():
        print(f"{name:45s} {tuple(tensor.shape)}")

def plot_losses(losses_record, save_path='loss_curve_v7.2.png'):
...
def save_losses_json(losses_record):
    with open('losses_record_v7.2.json', 'w') as f:
