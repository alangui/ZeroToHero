...
def train_loop():
    ...
    for iter in range(max_iters):
        ...
        if (iter+1) % save_model_interval == 0:    
            torch.save(model.state_dict(), f'model_ckpt_zh_v10_{iter+1}.pt')    

    t_total = time.time() - t_start
    logging.info(f"全部训练完成，总耗时 {t_total/60:.1f} 分钟，" f"平均每步 {t_total/max_iters*1000:.0f} 毫秒")

    torch.save(model.state_dict(), f'model_final_zh_v10.pt')
...
def load_model_for_inference():
    model = BigramLanguageModel()
    model.load_state_dict(torch.load('model_final_zh_v10.pt', map_location='cpu'))
    model.to(device)
    model.eval()
...
def analy_model():
    ckpt = torch.load('model_final_zh_v10.pt', map_location='cpu', weights_only=True)

    for name, tensor in ckpt.items():
        print(f"{name:45s} {tuple(tensor.shape)}")

def plot_losses(losses_record, save_path='loss_curve_v10.png'):
